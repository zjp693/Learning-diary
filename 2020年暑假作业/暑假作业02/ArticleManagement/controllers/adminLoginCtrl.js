...
module.exports.loginUser = (req, res) => {
  ...
  const schema = Joi.object().keys({
    username: Joi.string().min(3).max(6).required().error(new Error('用户名或密码不正确')),
    password: Joi.string().regex(/^[a-zA-Z0-9]{3,11}$/).required().error(new Error('用户名或密码不正确'))
  })
